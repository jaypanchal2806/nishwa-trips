...
app = FastAPI(title="Nishwa Travels API")
...
class BookingCreate(BaseModel):
    ...
    car_type: str = Field(..., description="Swift Dzire | Ertiga | Innova Crysta")
    trip_type: str = Field(..., description="One Way | Round Trip")
...
@api_router.get("/")
async def root():
    return {"message": "Nishwa Travels API is running"}
...
async def admin_update_booking(booking_id: str, patch: BookingUpdate, _: str = Depends(require_admin)):
...
async def admin_delete_booking(booking_id: str, _: str = Depends(require_admin)):
...
async def admin_stats(_: str = Depends(require_admin)):
    ...
    today = await db.bookings.count_documents({"created_at": {"$regex": f"^{today_iso}"}})
    return {
        "total":     total,
        "new":       new,
        "confirmed": confirmed,
